...
export class KitchenComponent extends Dialog implements OnInit {
  ...
  override ngOnInit(): void {
    this.orderService.findAllBeingPrepared().subscribe((orders: Order[]) => {
      let data: Order[] = []
  ...
  sendBackToWaiter(order: Order): void {
    this.orderService.updateStatePrepared(order).subscribe((response) => {
      this.ngOnInit()
    })
  }
