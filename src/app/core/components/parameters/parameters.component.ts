...
import { ParametersService } from '../../http/parameters.service';
...
export class ParametersComponent extends Dialog implements OnInit {
  ...
  override ngOnInit(): void {
    ...
    this.parametersService.getCurrentIp().subscribe((res: any) => {
  ...
  addUser = (): void => {
    ...
    this.userService.create(newUser).subscribe((data: any) => {
  ...
  deleteUser = (args: any[]): void => {
    ...
    this.userService.delete(userToDelete).subscribe((data: any) => {
  ...
  addRole = (): void => {
    if (this.newRoleForm.invalid)
      return
    this.roleService.create(this.newRoleForm.get("role")?.value || "").subscribe(data => {
